...
interface IProps {
  ...
  setSelectedCoin: Dispatch<SetStateAction<ISelectedCryptoCoin | null>>;
  coinIcon?: string | null;
  cryptoData: ICryptoCurrency[] | null;
  selectTag: keyof ISelectTags;
  currencyName?: string | null;
  setMinAmountFrom?: Dispatch<SetStateAction<string | null>>;
}
...
const CustomSelect = ({
  ...
  useEffect(() => {
    if(currencyName && searchValue && currencyName !== searchValue){
        setSearchValue(currencyName.toLocaleUpperCase())
    }
  },[currencyName])

  useEffect(() => {
    if(currencyName) setSearchValue(currencyName?.toLocaleUpperCase());
  }, [blur]);
